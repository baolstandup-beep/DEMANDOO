...
export const TripCard = ({ trip, index = 0 }) => {
  ...
  return (
    <div
      ...
      className={`
        group relative bg-white/10 backdrop-blur-xl rounded-3xl p-6 border border-white/20 shadow-lg 
        ...
        hover:shadow-2xl hover:border-white/40 hover:-translate-y-[3px]
      `}
    >
      <style>{`
        @media (hover: none) {
      ...
      <div className="flex justify-between items-start">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            <h4 className="font-black text-white text-sm flex items-center gap-1 line-clamp-1">
              <span className="truncate">{trip.driver?.full_name}</span>
              {trip.driver?.driver_status === 'VERIFIED' && <ShieldCheck className="w-3.5 h-3.5 text-emerald-400 shrink-0" title="Profil vérifié" />}
            </h4>
        ...
        <div className="bg-white/20 text-white px-3 py-1 rounded-full font-black text-xs border border-white/30 shrink-0 ml-2 shadow-sm backdrop-blur-md">
          {trip.seats_available} {trip.seats_available > 1 ? 'places' : 'place'}
        </div>
      </div>

      <div className="relative pl-6 space-y-3 my-2 border-l-2 border-white/20">
        <div className="relative">
          <div className="absolute -left-[29px] top-1 w-4 h-4 rounded-full bg-demandoo-400 border-4 border-white/20 shadow-sm" />
          <p className="font-black text-white line-clamp-1">{trip.departure_city}</p>
          <p className="text-xs text-white/70 font-medium">
        ...
        {Array.isArray(trip.waypoints) && trip.waypoints.length > 0 && (
          <div className="text-[11px] font-bold text-white/80 flex items-center gap-1.5 bg-white/10 py-1 px-2.5 rounded-lg w-fit border border-white/10">
            <MapPin className="w-3 h-3 text-demandoo-400" />
            <span>Via : {trip.waypoints.join(', ')}</span>
          </div>
        )}

        <div className="relative">
          <div className="absolute -left-[29px] top-1 w-4 h-4 rounded-full bg-slate-800 border-4 border-white/20 shadow-sm" />
          <p className="font-black text-white line-clamp-1">{trip.arrival_city}</p>
        </div>
      </div>
      ...
      <div className="pt-4 border-t border-white/20 flex items-center justify-between gap-3 mt-auto">
        ...
        <div className="flex items-center gap-2">
          <a
            ...
            className="w-10 h-10 rounded-xl bg-[#25D366]/20 text-[#25D366] hover:bg-[#25D366] hover:text-white transition-all flex items-center justify-center shrink-0 border border-[#25D366]/30 backdrop-blur-md"
          ...
          <Link 
            to={`/trajet/${trip.id}`} 
            className="min-h-[40px] px-4 py-2 rounded-xl bg-white/20 border border-white/30 backdrop-blur-md text-white text-xs font-black hover:bg-white/30 transition-all active:scale-95 flex items-center justify-center shrink-0 shadow-lg"
